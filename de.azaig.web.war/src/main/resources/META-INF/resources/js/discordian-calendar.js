 ...
 * 
 * lang: JavaScript author: jklu contributors: JamesMcGuigan
 * 
 * changelog: - Modified to return same output syntax as unix ddate +
 * module.exports - James McGuigan, 2/Chaos/3183
 * 
...
var weekday = [ "Sweetmorn", "Boomtime", "Pungenday", "Prickle-Prickle",
		"Setting Orange" ];

var apostle = [ "Mungday", "Mojoday", "Syaday", "Zaraday", "Maladay" ];

var holiday = [ "Chaoflux", "Discoflux", "Confuflux", "Bureflux", "Afflux" ];
...
function discordianDate(date) {
	...
	var nth = (seasonDay % 10 == 1) ? 'st' : (seasonDay % 10 == 2) ? 'nd'
			: (seasonDay % 10 == 3) ? 'rd' : 'th';

	return "" // (date.isToday() ? "Today is " : '')
			+ dayOfWeek + ", the " + seasonDay + nth + " day of " + season
			+ " in the YOLD " + yold
			+ (celebrateHoliday ? ". Celebrate " + celebrateHoliday + "!" : '');
}

function test(y, m, d, result) {
	console.assert((discordianDate(new Date(y, m, d)) == result), [ y, m, d,
			discordianDate(new Date(y, m, d)), result ]);
}
